...
def homepage(request):
    ...
    questionnaires = Questionnaire.objects.select_related('subject', 'category', 'topic', 'subtopic')
    total_questionnaires = questionnaires.count()

    restricted_questions = questionnaires.filter(status=1).select_related('subject', 'category', 'topic')
    restricted_counts = restricted_questions.count()
    ...
    restricted_by_category_raw = (
        restricted_questions
        .values('category__category')
        .annotate(total=Count('id'))
    )
    restricted_by_category = {key: 0 for key, _ in bloom_categories}
    for row in restricted_by_category_raw:
        category_key = (row.get('category__category') or "").lower()
        if category_key in restricted_by_category:
            restricted_by_category[category_key] = row['total']

    bloom_labels = [label for _, label in bloom_categories]
    bloom_data = [restricted_by_category[key] for key, _ in bloom_categories]
    restricted_percentage = round((restricted_counts / total_questionnaires) * 100, 2) if total_questionnaires else 0

    assessment_records = AssessmentRecordsDashboard.objects.none()
    assessment_summaries = []
    if active_year:
        assessment_records = (
            AssessmentRecordsDashboard.objects
            .filter(academic_year=active_year)
            .order_by('-category_total', '-assessment_id')
        )
        for record in assessment_records:
            summary = {
                'id': record.assessment_id,
                'label': record.topic or f"Assessment {record.assessment_id}",
                'subject_code': record.subject_code,
                'remembering': round(record.calculate_remembering_percentage, 2),
                'creating': round(record.calculate_creating_percentage, 2),
                'understanding': round(record.calculate_understanding_percentage, 2),
                'applying': round(record.calculate_applying_percentage, 2),
                'analyzing': round(record.calculate_analyzing_percentage, 2),
                'evaluating': round(record.calculate_evaluating_percentage, 2),
                'average': round(record.calculate_overall_percentage, 2),
            }
            assessment_summaries.append(summary)

    assessment_highlights = sorted(assessment_summaries, key=lambda item: item['average'], reverse=True)[:5]

    enrolled_students_count = (
        Students.objects.filter(academic_year=active_year).count()
        if active_year
        else Students.objects.count()
    )

    category_summary = (
        CategoriesCountPercentage.objects.filter(academic_year=active_year).first()
        if active_year
        else None
    )

    overall_percentages = {
        'remembering': round(category_summary.calculate_remembering_percentage, 2) if category_summary else 0,
        'creating': round(category_summary.calculate_creating_percentage, 2) if category_summary else 0,
        'understanding': round(category_summary.calculate_understanding_percentage, 2) if category_summary else 0,
        'applying': round(category_summary.calculate_applying_percentage, 2) if category_summary else 0,
        'analyzing': round(category_summary.calculate_analyzing_percentage, 2) if category_summary else 0,
        'evaluating': round(category_summary.calculate_evaluating_percentage, 2) if category_summary else 0,
        'average': round(category_summary.calculate_overall_percentage, 2) if category_summary else 0,
    }

    subject_data = []
    if active_year:
        subject_count_percentage_data = (
            SubjectCountPercentage.objects
            .filter(academic_year=active_year)
            .select_related('subject')
        )
        for subject in subject_count_percentage_data:
            subject_data.append({
                'subject_code': subject.subject.subject_code if subject.subject else 'N/A',
                'percentage': round(subject.calculate_cor_percentage(), 2),
                'total_questions': subject.total_q_counts_per_subject,
                'total_correct': subject.total_correct_counts_per_subject,
            })

    subject_data.sort(key=lambda item: item['percentage'], reverse=True)
    subject_highlights = subject_data[:4]
    subject_codes = [data['subject_code'] for data in subject_data]
    subject_percentages = [data['percentage'] for data in subject_data]

    restricted_subject_breakdown = []
    restricted_subject_rows = (
        restricted_questions
        .values('subject__subject_code')
        .annotate(total=Count('id'))
        .order_by('-total')[:5]
    )
    for row in restricted_subject_rows:
        restricted_subject_breakdown.append({
            'subject_code': row.get('subject__subject_code') or 'N/A',
            'total': row['total'],
        })

    recent_restricted = list(
        restricted_questions.order_by('-id')[:5]
    )

    status_value = request.session.get('status_value')

    context = {
        'active_year_label': str(active_year) if active_year else 'No active academic year',
        ...
        'restricted_percentage': restricted_percentage,
        'bloom_labels': bloom_labels,
        'bloom_data': bloom_data,
        'restricted_subject_breakdown': restricted_subject_breakdown,
        'recent_restricted': recent_restricted,
...
def addquestion(request):
    if request.method == 'POST':
        description_raw = request.POST.get('descriptioncreate', '')
        normalized_description = ''.join(description_raw.lower().split())
        ...
        existing_question = Questionnaire.objects.annotate(
            normalized=Replace(
                Replace(
                    Replace(
                        Replace(Lower('description'), Value(' '), Value('')),
                        Value('\n'),
                        Value('')
                    ),
                    Value('\t'),
        ...
        ).filter(normalized=normalized_description)
        ...
        try:
            ...
            logger.exception('Failed to create questionnaire item due to integrity error.')
            messages.error(request, 'Save failed due to a system error. Please try again or contact support.')
        else:
...
def updatequestion(request, id):
    if request.method == 'POST':
        try:
            ...
            e_raw = request.POST.get('descriptionupdate', '')
            normalized_description = ''.join(e_raw.lower().split())
            ...
            duplicate_question = Questionnaire.objects.annotate(
                normalized=Replace(
                    Replace(
                        Replace(
                            Replace(Lower('description'), Value(' '), Value('')),
            ...
            ).exclude(id=id).filter(normalized=normalized_description)

            if duplicate_question.exists():
                messages.error(request, 'This question already exists!')
            ...
            logger.exception('Failed to update questionnaire item due to integrity error.')
            messages.error(request, 'Update failed due to a system error. Please try again or contact support.')
            return redirect(reverse('update', args=[id]))
...
def print_questionnaire(request, group_id):
    ...
    tos_entries = list(TableOfSpecification.objects.filter(group_id=group_id))
    rd.shuffle(tos_entries)
    ...
    base_categories = list(category_counts.keys())
    TARGET_ITEM_COUNT = 100

    def normalize_required_items(raw_count):
        if raw_count in (None, ""):
            return 0
        try:
            return int(Decimal(raw_count))
    ...
    for entry in tos_entries:
        ...
        shuffled_categories = base_categories[:]
        rd.shuffle(shuffled_categories)

        if len(question_answer_pairs) >= TARGET_ITEM_COUNT:
            break

        for category in shuffled_categories:
            required_items = normalize_required_items(getattr(entry, category, 0))
            if required_items <= 0:
                continue

            remaining_slots = TARGET_ITEM_COUNT - len(question_answer_pairs)
            if remaining_slots <= 0:
                break

            items_to_take = min(required_items, remaining_slots)
            ...
            rd.shuffle(available_questions)

            selected_count = 0

            for candidate in available_questions:
                if candidate.id in used_question_ids:
                    continue

                choices = [
                    candidate.correct_answer,
                    candidate.distructor1,
                    candidate.distructor2,
                    candidate.distructor3,
                ]
                rd.shuffle(choices)

                correct_choice_letter = get_correct_choice_letter_tos(candidate, choices)
                ...
                answer_key_entry = AnswerKeyTableOfSpecification(
                ...
                question_answer_pairs.append((question_entry, answer_key_entry))

                used_question_ids.add(candidate.id)
                selected_count += 1

                if selected_count >= items_to_take:
                    break
    ...
    if len(question_answer_pairs) < TARGET_ITEM_COUNT:
        messages.error(
            request,
            "Unable to generate 100 questions. Please ensure each category has enough available items.",
        )

    total_pairs = len(question_answer_pairs)

    if total_pairs >= TARGET_ITEM_COUNT:
        final_pairs = rd.sample(question_answer_pairs, TARGET_ITEM_COUNT)
    elif total_pairs > 0:
        final_pairs = rd.sample(question_answer_pairs, total_pairs)
    else:
        final_pairs = []

    if final_pairs:
        rd.shuffle(final_pairs)

    for index, (question_entry, answer_key_entry) in enumerate(final_pairs, start=1):
        question_entry['number'] = index
        generated_questions.append(question_entry)
        answer_key_entry.number = index
        answer_keys_tos.append(answer_key_entry)

        category_counts[question_entry['category']] += 1
        topic_counts[question_entry['topic']] += 1
        subtopic_counts[question_entry['subtopic']] += 1
...
def print_assessment(request, assessment_id):
    ...
    rd.shuffle(question_entries)

    generated_questions = []
    answer_data = []

    for question_number, entry in enumerate(question_entries, start=1):
        lettered_choices = [(chr(65 + i), choice) for i, choice in enumerate(entry['choices'])]
        ...
        answer_data.append({
            ...
            'c': entry['choices'][2],
            'd': entry['choices'][3],
            'correct_choice': entry['correct_choice_letter'],
            'correct_answer': entry['correct_answer'],
        })
        ...
        subtopic_counts[entry['subtopic']] += 1
